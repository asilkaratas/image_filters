...
import com.imagefilters.util.PixelUtil;
...
{
    @Override
    public void fill(byte[] enlargedPixels, byte[] originalPixels, int originalWidth, int originalHeight, int enlargedWidth, int enlargedHeight, Point pivot,  int kernelWidth, int kernelHeight)
    {
        //top
        int top = pivot.y;
        
        Rectangle topRectangle = new Rectangle(0, 0, originalWidth, 1);
        byte[] topPixels = PixelUtil.copyPixelsFromRect(originalPixels, originalWidth, originalHeight, topRectangle);
        ...
        byte[] bottomPixels = PixelUtil.copyPixelsFromRect(originalPixels, originalWidth, originalHeight, bottomRectangle);
        ...
        byte[] leftPixels = PixelUtil.copyPixelsFromRect(enlargedPixels, enlargedWidth, enlargedHeight, leftRectangle);
        ...
        byte[] rightPixels = PixelUtil.copyPixelsFromRect(enlargedPixels, enlargedWidth, enlargedHeight, rightRectangle);
